...
// Loads OGL's UMD build with local-first fallbacks so everything "just works".

// --- Load OGL UMD (window.OGL) with robust fallbacks ---
...
async function ensureOGL() {
  ...
  const CANDIDATES = [
    // 1) Your repo (same folder as this file when served via jsDelivr/GitHub Pages)
    new URL('./ogl.umd.js', import.meta.url).href,

    // 2) Your raw GitHub file (as you provided)
    'https://raw.githubusercontent.com/GDS-stack/world-map-data/refs/heads/main/ogl.umd.js',

    // 3) Known CDN mirrors that ship /dist/ogl.umd.js
  ...
  throw lastErr || new Error('Could not load OGL from any source.');
}
...
export default async function initRipple(opts = {}) {
  const {
    ...
    dataUrl = '',
    targetDotPx = 4,     // ~4px squares on screen
    cornerPct = 0.12,    // 0..0.5 (of half-size) for rounded corners
    seedFraction = 0.20, // fraction of points that act as ripple seeds
    cycleSec = 10,       // seconds per ripple cycle
    rngJitter = 0.5,     // seconds ± jitter to desync rings
    maxSeeds = 64,       // cap for seeds (uniform budget)
    powerPreference = 'high-performance'
  } = opts;

  // ---- Canvas ----
  const canvas = document.querySelector(canvasSelector);
  if (!canvas) { console.error('[wm] Canvas not found:', canvasSelector); return; }

  // ---- Load OGL ----
  const OGL = await ensureOGL();
  ...
  const cssHexToLinearRGB = (hex) => {
    ...
    const toLin = v => Math.pow(v, 2.2); // simple sRGB->linear approx
  ...
  // ---- Load data ----
  if (!dataUrl) { console.error('[wm] Missing dataUrl.'); return; }
  let pts;
  try {
    ...
    console.error('[wm] Data fetch failed:', dataUrl, err);
    return;
  }
  if (!Array.isArray(pts) || !pts.length) { console.error('[wm] Data empty/invalid'); return; }

  // ---- Bounds (like SVG viewBox with padding) ----
  ...
  // ---- Seeds (same semantics as CSS version) ----
  ...
  // Wave crosses most of the map in ~60% of cycle
  ...
  const fragment = /* glsl */`
    ...
    // Ripple timing (matches your CSS: quick in, brief hold, long tail)
  ...
  // ---- Data->NDC transform (flip Y to feel like SVG) ----
