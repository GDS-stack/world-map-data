// wm-ripple-gl.js — Standalone WebGL2 ripple renderer (no OGL dependency)
...
function createProgram(gl, vertSrc, fragSrc, attribBind = null) {
  const vs = createShader(gl, gl.VERTEX_SHADER, vertSrc);
  const fs = createShader(gl, gl.FRAGMENT_SHADER, fragSrc);
  const prg = gl.createProgram();
  gl.attachShader(prg, vs);
  gl.attachShader(prg, fs);
  if (attribBind) attribBind(prg, gl);
  gl.linkProgram(prg);
  gl.deleteShader(vs);
  gl.deleteShader(fs);
  if (!gl.getProgramParameter(prg, gl.LINK_STATUS)) {
    const log = gl.getProgramInfoLog(prg) || 'link failed';
...
export default async function initRipple(opts = {}) {
  const {
    ...
    dataUrl = '',
    targetDotPx = 4,
    cornerPct = 0.12,     // 0..1 (we map to 0..0.5 inside shader)
    seedFraction = 0.20,
    cycleSec = 10,
    rngJitter = 0.5,
    maxSeeds = 64,
  ...
  if (!gl) {
    console.error('[wm] WebGL2 not available.');
    return;
  }
  ...
  // 3) Load data
  if (!dataUrl) { console.error('[wm] dataUrl missing'); return; }
  let pts;
  try {
    ...
    console.error('[wm] Data fetch failed:', e?.message || e);
    return;
  }
  if (!Array.isArray(pts) || !pts.length) {
    console.error('[wm] Data invalid/empty');
    return;
  }
  ...
  // 5) Seed selection & per-instance phase delay
  ...
  // 6) Build geometry (unit quad as 2 triangles)
  ...
  const vertSrc = `#version 300 es
    ...
    void main() {
      ...
      float s = uBounds.z;               // side length in data units
  ...
  const fragSrc = `#version 300 es
    ...
    float rippleWindow(float tNorm){
      if (tNorm < 0.03) return smoothstep(0.0, 0.03, tNorm); // quick fade-in
      else if (tNorm < 0.10) return 1.0;                     // brief bright ring
      float k = (tNorm - 0.10) / 0.90;                       // long fade out
      return 1.0 - k;
    }
  ...
  gl.uniform1f(uCorner, cornerPct * 0.5); // map 0..1 to 0..0.5 (half-size space)
  gl.uniform3fv(uIdle,   IDLE);
  gl.uniform3fv(uBright, BRIGHT);

  // transform (SVG-like viewBox mapping to NDC)
  let dotSizeData = 1;
  ...
  // 9) Resize handling (CSS pixels -> device pixels)
  function resize() {
  ...
  gl.disable(gl.BLEND); // opaque squares
  gl.clearColor(0,0,0,0);

  // respect reduced motion + intersection
  let running = true;
